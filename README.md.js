var Fir_id = nazSet["Fir-id"] || "",
    MegaMax = nazSet["mega-max"] || 10,
    errTrans = false;
...
function popUp(e, t) {
    return e.preventDefault(), window.open(t.href, "_blank", "height=570,width=600"), false
}
for (var SubMitems = document.querySelectorAll("#" + menu_instanceId + " .sub, #" + menu_instanceId + " .ssub"), si = 0; si < SubMitems.length; si++) SubMitems[si].querySelector("a").innerHTML = SubMitems[si].querySelector("a").innerHTML.replace(/_/g, "");
for (var MainMitems = document.querySelectorAll("#" + menu_instanceId + " .drop-menu, #" + menu_instanceId + " .bot-menu"), mi = 0; mi < MainMitems.length; mi++) {
    ...
    item.querySelector("ul") ? item.querySelector("ul li") || (item.removeChild(item.querySelector("ul")), item.removeAttribute("class")) : item.removeAttribute("class")
}
...
for (si = 0; si < SubMenus.length; si++) {
    ...
    item.onmouseover = function() {
        992 < window.innerWidth && (BlogDir == "rtl" ? this.getBoundingClientRect().left < 200 && (ul.style.right = "auto", ul.style.left = "200px") : window.innerWidth - this.getBoundingClientRect().right < 200 && (ul.style.left = "auto", ul.style.right = "200px"))
...
for (var MegaItems = document.querySelectorAll(".MegaItem"), gi = 0; gi < MegaItems.length; gi++) {
    ...
    item.setAttribute("data-label", label), item.href = "/search/label/" + label + "?max-results=" + MegaMax
}
...
for (pl = 0; pl < PostLinks.length; pl++) PostLinks[pl].addEventListener("click", function(c) {
    c.preventDefault()
});

function resizeImg(g, R, _) {
    try {
        var e = nazSet["sup-web"] || false
    } catch (t) {
        e = false
    }
    try {
    ...
    function g() {
        for (var g = document.querySelectorAll("img[" + i + "]"), R = 0; R < g.length; R++) {
            ...
            if (_.getBoundingClientRect().top - document.body.getBoundingClientRect().top < window.pageYOffset + window.innerHeight || a) {
                ...
                e = resizeImg(_.getAttribute(i), e, t), _.setAttribute("src", e), _.removeAttribute(i), _.removeAttribute("class"), _.removeAttribute("style"), _.parentNode.classList.remove("LazyLoad")
            }
        }
    }
    a ? g() : document.addEventListener("scroll", g)
}
if (LazyImages("data-src"), window.addEventListener("scroll", function() {
    if (100 <= window.pageYOffset && document.body.getAttribute("data-auth") != "true") {
        document.body.setAttribute("data-auth", "true");
        var g = document.createElement("link");
        g.rel = "stylesheet", g.href = "https://www.blogger.com/dyn-css/authorization.css?targetBlogID=" + blogId, document.head.appendChild(g), g = document.querySelectorAll(".qed");
        for (var R = 0; R < g.length; R++) {
            var _ = g[R].closest(".widget"),
                e = _.classList[1],
                t = _.getAttribute("id");
            _ = _.closest(".section").getAttribute("id"), g[R].setAttribute("class", "item-control blog-admin"), g[R].innerHTML = '<a class="quickedit" href="https://www.blogger.com/rearrange?blogID=' + blogId + "&widgetType=" + e + "&widgetId=" + t + "&action=editWidget&sectionId=" + _ + '" onclick="popUp(event,this);"></a>'
        }
    }
}), isStorage) if (null != sessionStorage.scripts) {
    var style = document.createElement("style");
    style.innerHTML = sessionStorage.fontawesome, document.head.appendChild("style"), document.querySelector("#Icons").innerHTML = "<svg>" + sessionStorage.icons + "</svg>", eval(sessionStorage.scripts)
} else GetScriptsAndIcons();
else GetScriptsAndIcons();
...
function GetScriptsAndIcons() {
    ...
    getFeeds.src = "https://www.blogger.com/feeds/4842518707706426847/posts/default/8307659015295844809?alt=json-in-script&callback=Scripts", document.head.appendChild(getFeeds), (getFeeds = document.createElement("script")).src = "https://www.blogger.com/feeds/4842518707706426847/posts/default/9080817711384057158?alt=json-in-script&callback=Icons", document.head.appendChild(getFeeds)
}
function Scripts(a) {
...
function Icons(getSvg) {
    ...
    R.innerHTML = getSvg.entry.content.$t, getSvg = R.getElementsByTagName("style")[0], document.head.appendChild(getSvg), sessionStorage.fontawesome = getSvg.innerHTML, R = R.getElementsByTagName("svg")[0], document.querySelector("#Icons").innerHTML = "<svg>" + R.innerHTML + "</svg>", sessionStorage.icons = R.innerHTML
}
